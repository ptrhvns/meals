...
def test_http_method_names() -> None:
    assert sets_http_method_names(food_create, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(food_create, permission_classes)


def test_invalid_request_data(api_rf: APIRequestFactory) -> None:
    request = api_rf.post(reverse("food_create"), {})
    user = UserFactory.build()
    authenticate(request, user)
    response = food_create(request)
    assert response.status_code == HTTP_422_UNPROCESSABLE_ENTITY
    assert len(response.data["errors"]) > 0  # type: ignore[index]
    assert len(response.data["message"]) > 0  # type: ignore[index]


def test_gets_existing_food(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.post(reverse("food_create"), {"name": "banana"})
    user = UserFactory.build()
    authenticate(request, user)
    mock = mocker.patch(
        "main.views.food_create.Food.objects.get_or_create",
        autospec=True,
        return_value=[False],
    )
    response = food_create(request)
...
def test_creates_food(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.post(reverse("food_create"), {"name": "banana"})
    user = UserFactory.build()
    authenticate(request, user)
    mock = mocker.patch(
