...
def test_http_method_names() -> None:
    assert sets_http_method_names(servings_update, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(servings_update, permission_classes)


def test_recipe_not_found(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    recipe_id = 1
    request = api_rf.post(reverse("servings_update", kwargs={"recipe_id": recipe_id}))
    user = UserFactory.build()
    authenticate(request, user)
    mocker.patch(
        "main.views.servings_update.get_object_or_404",
        autospec=True,
        side_effect=Http404,
    )
    response = servings_update(request, recipe_id)
...
def test_invalid_request_data(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    recipe_id = 1
    request = api_rf.post(
        reverse("servings_update", kwargs={"recipe_id": recipe_id}), {}
    )
    user = UserFactory.build()
    authenticate(request, user)
...
def test_successful_update(api_rf: APIRequestFactory) -> None:
    ...
    recipe = RecipeFactory.create(user=user, servings=1)
    new_servings = 4
    request = api_rf.post(
        reverse("servings_update", kwargs={"recipe_id": recipe.id}),
        {"servings": new_servings},
    )
    authenticate(request, user)
    response = servings_update(request, recipe.id)
    assert response.status_code == HTTP_204_NO_CONTENT
    recipe.refresh_from_db()
    assert new_servings == recipe.servings
