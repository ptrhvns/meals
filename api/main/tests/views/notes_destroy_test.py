...
def test_http_method_names() -> None:
    assert sets_http_method_names(notes_destroy, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(notes_destroy, permission_classes)


def test_not_found(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    recipe_id = 1
    request = api_rf.post(reverse("notes_destroy", kwargs={"recipe_id": recipe_id}))
    user = UserFactory.build()
    authenticate(request, user)
    mocker.patch(
        "main.views.notes_destroy.get_object_or_404",
        autospec=True,
        side_effect=Http404,
    )
    response = notes_destroy(request, recipe_id)
...
def test_destroyed_successfully(
    ...
    request = api_rf.post(reverse("notes_destroy", kwargs={"recipe_id": recipe.id}))
    authenticate(request, user)
    response = notes_destroy(request, recipe.id)
    assert response.status_code == HTTP_204_NO_CONTENT
    recipe.refresh_from_db()
    assert not recipe.notes
