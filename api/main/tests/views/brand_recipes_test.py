...
def test_http_method_names() -> None:
    assert sets_http_method_names(brand_recipes, ["get", "options"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(brand_recipes, permission_classes)


def test_brand_not_found(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.get(reverse("brand_recipes", kwargs={"brand_id": 1}))
    user = UserFactory.build()
    authenticate(request, user)
    mocker.patch(
        "main.views.brand_recipes.get_object_or_404",
        autospec=True,
        side_effect=Http404,
    )
    response = brand_recipes(request, 1)
...
def test_getting_recipes_successfully(api_rf: APIRequestFactory) -> None:
    ...
    response = brand_recipes(request, brand.id)  # pyright: ignore
