...
def test_http_method_names() -> None:
    assert sets_http_method_names(brand_create, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(brand_create, permission_classes)


def test_invalid_request_data(api_rf: APIRequestFactory) -> None:
    request = api_rf.post(reverse("brand_create"), {})
    user = UserFactory.build()
    authenticate(request, user)
    response = brand_create(request)
    assert response.status_code == HTTP_422_UNPROCESSABLE_ENTITY
    assert len(response.data["errors"]) > 0  # type: ignore[index]
    assert len(response.data["message"]) > 0  # type: ignore[index]


def test_gets_existing_brand(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.post(reverse("brand_create"), {"name": "Acme"})
    user = UserFactory.build()
    authenticate(request, user)
    mock = mocker.patch(
        "main.views.brand_create.Brand.objects.get_or_create",
        autospec=True,
        return_value=[False],
    )
    response = brand_create(request)
...
def test_creates_brand(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.post(reverse("brand_create"), {"name": "Acme"})
    user = UserFactory.build()
    authenticate(request, user)
    mock = mocker.patch(
