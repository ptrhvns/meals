...
def test_http_method_names() -> None:
    assert sets_http_method_names(rating_update, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(rating_update, permission_classes)


def test_rating_not_found(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    recipe_id = 1
    request = api_rf.post(reverse("rating_update", kwargs={"recipe_id": recipe_id}))
    user = UserFactory.build()
    authenticate(request, user)
    mocker.patch(
        "main.views.rating_update.get_object_or_404",
        autospec=True,
        side_effect=Http404,
    )
    response = rating_update(request, recipe_id)
...
def test_successful_update(api_rf: APIRequestFactory) -> None:
    ...
    recipe = RecipeFactory.create(user=user, rating=1)
    new_rating = 5
    request = api_rf.post(
        reverse("rating_update", kwargs={"recipe_id": recipe.id}),
        {"rating": new_rating},
    )
    authenticate(request, user)
    response = rating_update(request, recipe.id)
    assert response.status_code == HTTP_204_NO_CONTENT
    recipe.refresh_from_db()
    assert new_rating == recipe.rating
