...
def test_http_method_names() -> None:
    assert sets_http_method_names(direction_destroy, ["options", "post"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(direction_destroy, permission_classes)


def test_not_found(api_rf: APIRequestFactory, mocker: MockerFixture) -> None:
    request = api_rf.post(reverse("direction_destroy", kwargs={"direction_id": 1}))
    user = UserFactory.build()
    authenticate(request, user)
    mocker.patch(
        "main.views.direction_destroy.get_object_or_404",
        autospec=True,
        side_effect=Http404,
    )
    response = direction_destroy(request, 1)
...
def test_destroyed_successfully(
    ...
    mocker.patch.object(direction, "delete")
    mocker.patch(
        "main.views.direction_destroy.get_object_or_404",
        autospec=True,
    ...
    assert direction.delete.called  # pyright: ignore
