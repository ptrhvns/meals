...
def test_http_method_names() -> None:
    assert sets_http_method_names(tags, ["get", "options"])


def test_permission_classes() -> None:
    permission_classes = [IsAuthenticated]
    assert sets_permission_classes(tags, permission_classes)
...
def test_get_tag_with_no_page_number(
    ...
    request = api_rf.get(reverse("tags"))
    authenticate(request, user)
    tag = TagFactory.create_batch(size=15, user=user)
    ...
    data["tags"] = sorted(data["tags"], key=key)
    assert data == {
        "tags": sorted([{"id": b.id, "name": b.name} for b in tag], key=key),
    }
...
def test_get_tag_with_page_number(
    ...
    user = UserFactory.create()
    request = api_rf.get(reverse("tags"), {"page": 2})
    authenticate(request, user)
    tag = TagFactory.create_batch(size=15, user=user)
    response = tags(request)
    assert response.status_code == HTTP_200_OK
    ...
    assert data == {
        ...
        "tags": sorted([{"id": b.id, "name": b.name} for b in tag], key=key)[10:],
    }
